...
classNames = list(os.listdir(args["dataset_audio"]))

# load image dataset
s = SimpleDatasetLoader()
print("[INFO] loading images train data...")
tX, tY = s.load(args["dataset_image"], verbose=50)
print("[INFO] loading images validation data...")
vX, vY = s.load(args["dataset_image_val"], verbose=50)

# load audio dataset
print("[INFO] loading audio dataset...")
c = CustomDatasetLoaderAndSplitter(args["dataset_audio"], validation=0.2)
trainX, trainY, valX, valY = c.load_and_split()

trainX = np.vstack((trainX, tX))
trainY = np.hstack((trainY, tY))
valX = np.vstack((valX, vX))
valY = np.hstack((valY, vY))
...
print(classification_report(valY.argmax(axis=1), predictions.argmax(axis=1), target_names=classNames))
