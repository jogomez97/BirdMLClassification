...
class CustomFusionDatasetLoaderAndSplitter:
    ...
    def __init__(self, input_path, image_path, image_path_val, validation=0.25, test=0, verbose=False, included_folders=[], image_size=224, only_val=False):
        ...
        # Image preprocessors
        self.preprocessors = [AspectAwarePreprocessor(224, 224), ImageToArrayPreprocessor()]
    ...
    def __load_random_image(self, is_val, label):
            ...
            while True:
                ...
                try:
                    iPath = os.path.join(imagePath, random.choice(os.listdir(imagePath)))
                except Exception as e:
                    print("Error in image path:", imagePath)
                    print(e)
                    continue
                # iPath += '/' + random.choice(os.listdir(imagePath))
